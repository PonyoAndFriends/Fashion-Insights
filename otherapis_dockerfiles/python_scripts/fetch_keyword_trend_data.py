...
def fetch_fashion_keyword_data_and_load_to_s3(url, headers, keyword_batch, s3_dict):
    ...
    for i, (gender, second_category, third_category, keywords) in enumerate(keyword_batch):
        body = {
            "startDate": one_week_ago_string,
            "endDate": now_string,
            "timeUnit": "date",
            "category": "50000000",
            "keyword": [{"name": f"{gender}_{second_category}_{third_category}_trend", "param": [keyword]} for keyword in keywords],
        ...
        s3_dict['file_path'] = f"/keyword_trend_raw_data/{now_string}/{gender}_{second_category}_{third_category}_trend_{i}.json"
        s3_dict['content_type'] = 'application/json'
        s3_upload.load_data_to_s3(s3_dict)


def main():
    parser = argparse.ArgumentParser(description="Process keywords and upload API results to S3")
    ...
    parser.add_argument("--category_list", required=True, help="JSON string of keywords")
    parser.add_argument("--max_threads", type=int, default=15, help="Maximum number of threads")
    ...
    run_func_multi_thread.execute_in_threads(fetch_fashion_keyword_data_and_load_to_s3, args, max_threads)
