    ...
    def _fetch_paged_data(self):
        ...
        for page in range(start_page, end_page + 1):
            ...
            if self.content_type == "application/json":
                ...
                file_path = (
                    f"/{self.file_topic}_raw_data/{now}/{self.file_topic}_{page}.json"
                )
            elif self.content_type == "text/html":
                data_file = response.text
                file_path = (
                    f"/{self.file_topic}_raw_data/{now}/{self.file_topic}_{page}.html"
                )
            else:
                data_file = response.text
                file_path = (
                    f"/{self.file_topic}_raw_data/{now}/{self.file_topic}_{page}.txt"
                )
        ...
        self.log.info(
            f"Successfully fetched data for pages {start_page} to {end_page}."
        )
