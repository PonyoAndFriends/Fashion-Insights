...
with DAG(
    dag_id='musinsa_snap_api_ranking_history_to_s3_dag',
    ...
    def fetch_paginated_data_task(page_range, gender):
        ...
        for page in range(start_page, end_page + 1):
            ...
            # JSON 데이터를 S3에 저장
            s3_file_path = f"/musinsa_snap_raw_data/{date_string}/snap_ranking_histroy_{page}.json"
            s3.put_object(
        ...
        print(f"Fetched {PAGE_SIZE} datas per each {end_page - start_page + 1} pages from Musinsa SNAP Ranking history api.")
