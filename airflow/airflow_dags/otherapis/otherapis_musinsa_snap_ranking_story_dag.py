...
BRONZE_FILE_PATH = f"bronze/{datetime.now().strftime('%Y-%m-%d')}/otherapis/{FILE_TOPIC}_raw_data/"
SILVER_FILE_PATH = f"silver/{datetime.now().strftime('%Y-%m-%d')}/otherapis/{FILE_TOPIC}_raw_data/"
...
with DAG(
    ...
    for gender in ["MEN", "WOMEN"]:
        file_topic = f"musinsa_{gender}_ranking_story_group"
        file_path = (
            f"{datetime.now().astimezone(ZoneInfo("Asia/Seoul")).strftime('%Y-%m-%d')}/otherapis/{file_topic}_raw_data/"
        )
