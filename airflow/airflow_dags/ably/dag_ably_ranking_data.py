    ...
    def collect_and_upload_data(self, category_id, sub_category_id, data_type):
        ...
        with self.semaphore:  # 세마포어로 동시 실행 제한
            try:
                ...
                # API 호출
                response = requests.get(self.api_url, headers=self.api_headers, params=payload, timeout=10)
                ...
                if data_type == 'ranking':
                    ...
                    goods_sno_list = [
                        ...
                        if 'logging' in item and 'analytics' in item['logging'] and 'GOODS_SNO' in item['logging']['analytics']
                    ]
                ...
                # 오류 발생 시 로그 출력
                logger.error(f"Error uploading {data_type} data for category {category_id}, sub_category {sub_category_id}: {e}")
