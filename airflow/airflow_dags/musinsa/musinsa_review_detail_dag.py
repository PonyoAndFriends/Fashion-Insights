...
def split_product_ids(**context):
    ...
    product_ids = context["dag_run"].conf.get("product_ids", [])  # List of 50 product IDs

    # 5개의 
    chunk_size = math.ceil(len(product_ids) / PARALLEL_TASK_NUM)  # 
    product_id_chunks = [product_ids[i:i + chunk_size] for i in range(0, len(product_ids), chunk_size)]
